...
import InputToArray from "./InputToArray.vue";
...
function submitChanges(){
	const changes = dataArrayCopy.value.filter(data => data.isEditing)
		.reduce((accumulator, data) => {
			if(data.type === "object"){
				accumulator[data.key] = data.value.currValue;
			}
			else{
	...
	emit("profileData:action", {
		...
		data: {
			...
			dataArrayCopy: dataArrayCopy.value.map(data => toRaw(data)),
		}
	});
...
<template>    
	<ul class="profile-data-container">
		<li class="data" v-for="(data, index) in dataArrayCopy">
			...
			<div v-if="data.type === 'object'" class="data-container">
			...
			<div v-if="data.type === 'string'" class="data-container">
				<span>{{data.label}}:</span>
			...
			<div v-if="data.type === 'array'" class="data-container">
				<span>{{data.label}}:</span>
				<span v-if="!data.isEditing">{{data.value}}</span>
				<InputToArray v-if="data.isEditing"></InputToArray>
				<Button 
					v-if="!data.isEditing"
					variant="outlined"
					rounded
